...
class AttnRateCell(JaxComponent): ## Rate-coded/real-valued cell
    """
    ...
    def __init__(
        ...
        self.jv = Compartment(restVals, display_name="Input Stimulus Current v", units="mA") 
        self.jq = Compartment(restVals, display_name="Input Stimulus Current q", units="mA") 
        self.jk = Compartment(restVals, display_name="Input Stimulus Current k", units="mA") 
    ...
    def advance_state(self, dt):
        ...
        if self.is_stateful:
            ...
            dfx_val = self.dfx(z)
            jv = _modulate(jv, dfx_val) * self.resist_scale
            jq = _modulate(jq, dfx_val) * self.resist_scale
            jk = _modulate(jk, dfx_val) * self.resist_scale
    ...
    def reset(self): 
        ...
        self.jv.set(restVals)
        self.jq.set(restVals)
        self.jk.set(restVals)
